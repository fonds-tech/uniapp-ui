<template>
  <demo-page>
    ...
    <demo-section title="基础用法">
      <demo-block direction="column">
        <ui-collapse v-model="activeBasic" @change="onChange">
          <ui-collapse-item name="1" title="标题一">
            代码是写出来给人看的，附带能在机器上运行。
          </ui-collapse-item>
          <ui-collapse-item name="2" title="标题二">
            技术无非就是那些开发它的人的共同灵魂。
          </ui-collapse-item>
          <ui-collapse-item name="3" title="标题三">
    ...
    <!-- 默认展开 -->
    <demo-section title="默认展开">
      <demo-block direction="column" align="start" :gap="12">
        <text class="demo-text">当前展开: {{ JSON.stringify(activeDefault) }}</text>
        <ui-collapse v-model="activeDefault">
          <ui-collapse-item name="1" title="默认展开项">
            通过 v-model 绑定默认展开的面板。
          </ui-collapse-item>
          <ui-collapse-item name="2" title="标题二">
            可以设置多个默认展开项。
          </ui-collapse-item>
          <ui-collapse-item name="3" title="也是默认展开项">
            传入数组可以同时展开多个面板。
          </ui-collapse-item>
        </ui-collapse>
      </demo-block>
    ...
    <demo-section title="禁用状态">
      <demo-block direction="column">
        <ui-collapse v-model="activeDisabled">
          <ui-collapse-item name="1" title="正常项">
            这是一个正常的折叠项。
          </ui-collapse-item>
          <ui-collapse-item name="2" title="禁用项" disabled>
            这个折叠项已被禁用，无法点击展开。
          </ui-collapse-item>
          <ui-collapse-item name="3" title="正常项">
            这是另一个正常的折叠项。
          </ui-collapse-item>
        </ui-collapse>
      </demo-block>
    ...
    <demo-section title="自定义标题">
      <demo-block direction="column">
        <ui-collapse v-model="activeCustomTitle">
          ...
          <ui-collapse-item name="2" title="普通标题">
            这是一个普通标题的折叠项。
          </ui-collapse-item>
        </ui-collapse>
      </demo-block>
    ...
    <demo-section title="事件处理">
      <demo-block direction="column" align="start" :gap="12">
        ...
        <ui-collapse v-model="activeEvent" @change="onChangeWithLog">
          <ui-collapse-item name="1" title="标题一" @click="onItemClick(1)">
            点击折叠项会触发 click 事件。
          </ui-collapse-item>
          <ui-collapse-item name="2" title="标题二" @click="onItemClick(2)">
            展开/收起会触发 change 事件。
          </ui-collapse-item>
          <ui-collapse-item name="3" title="标题三" @click="onItemClick(3)">
            可以监听事件进行自定义处理。
          </ui-collapse-item>
        </ui-collapse>
      </demo-block>
...
function onChangeWithLog(value: CollapseModelValue) {
  ...
  eventLog.value = `change: ${displayValue}`
